...
| _Ussage_ | How are service workers used in PWA? |
...
**What we'll cover today:** We'll explore the remaining building blocks (HTTPS, Service Workers) with specific focus on [using Service Workers](https://docs.microsoft.com/en-us/microsoft-edge/progressive-web-apps-chromium/how-to/service-workers).
 ...
 * The **Service Worker** is the Chief Operations Officer (COO) - it unblocks the CEO from performing time-consuming or asynchronous tasks, and takes proactive actions to ensure reliable operation even when offline.

![Placeholder Banner Only. Replace when final assets ready.](_media/4-service-workers.png)
...
**HTTPS support is mandatory for using Service Workers**. Thankfully, as we covered [in our earlier post](02.md), it is easy to implement HTTPS support. Use modern cloud hosting providers (who enable it by default) or take advantage of free certificate options (e.g., [Let's Encrypt](https://letsencrypt.com) to secure your own servers.


## Make PWAs Reliable & Re-Engageable

This is achieved with Service Workers.

Service Workers are a special type of [Web Worker](https://aka.ms/learn-PWA/30Days-1.2/www.w3schools.com/js/js_api_web_workers.asp) that execute long-running or asynchronous tasks in the background, minimizing impact on page performance ("unblocking" the CEO).
...
Service Workers make PWAs _re-engageable_ by having the ability to alert users to app changes or context, even if the page itself is inactive. They do this by listening for asynchronous push notifications (from server) and working with platform capabilities to deliver alerts in a device-familiar way. When users engage with the alert, they are drawn back into the app seamlessly - just like with other native app experiences.
...
Let's look at registration first. The Service Worker functionality must be implemented in its own file. The location of that file (relative to the root of the app) defines the _scope_ of its authority. Service Workers can only intercept or manage requests to pages within their scope. _Ideally, placing the file at the root of your app ensures service workers will manage all pages within it_.

Let's inspect our [DevTools Tips PWA](https://devtoolstips.org) in the browser, and look at Service Workers under the _Application_ tab. We can see that the service worker is implemented in the "sw.js" file in the root directory - implicitly setting its scope to the whole app.
...
Because Service Worker is a more recent technology that may not be [supported on all browsers](https://caniuse.com/serviceworkers), we test for its existence before registering it. The scope is set implicitly by the file location - the code shows how you can explicitly set this if needed. 
...
1. **Registration**: The browser registers the service worker. It is now ready to receive and process events.

2. **Installation**: The browser triggers `install` as the first event to the Service Worker. It can use this for pre-caching resources (e.g., populate cache with long-running resources like logos or offline pages) while it waits for older service worker versions to exit.
...
3. **Activation**: The browser sends the `activate` event to indicate that prior service worker versions have exited. This service worker can now do clean up actions (e.g., remove caches from prior version) and ready itself to handle _functional_ events.
...
2. The [**push event**](https://developer.mozilla.org/en-US/docs/Web/API/PushEvent) is triggered when the browser receives a push message from a server to display as a toast notification to users. This occurs only if the PWA had previously _subscribed_ for server notifications _and_ user had granted PWA permissions to receive them. Push events are critical to [re-engaging users](https://docs.microsoft.com/en-us/microsoft-edge/progressive-web-apps-chromium/how-to/notifications-badges) when the app is not otherwise active.
...
Use DevTools to inspect a  a different [sample PWA](https://aka.ms/learn-PWA/30Days-1.1/docs.microsoft.com/en-us/microsoft-edge/progressive-web-apps-chromium/demo-pwas) and see if you can identify and understand the service worker hooks and implementation:

* Go to `Elements` tab and explore the application source
    - Where is service worker registered?
