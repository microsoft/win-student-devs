...
Service Workers are a special type of [Web Worker](https://aka.ms/learn-PWA/30Days-1.4/www.w3schools.com/js/js_api_web_workers.asp) that execute long-running or asynchronous tasks in the background, minimizing impact on page performance ("unblocking" the CEO).
...
Let's inspect our [DevTools Tips PWA](https://aka.ms/learn-PWA/30Days-1.4/devtoolstips.org) in the browser, and look at Service Workers under the _Application_ tab. We can see that the service worker is implemented in the "sw.js" file in the root directory - implicitly setting its scope to the whole app.
