...
Plus, we picked a [Sample PWA](https://aka.ms/learn-PWA/30Days-1.1/docs.microsoft.com/en-us/microsoft-edge/progressive-web-apps-chromium/demo-pwas) to look at as we explore the concepts. Now, let's dive into PWA building blocks, starting with manifests.
...
The **Web App Manifest** is an open web [specification of a JSON format](https://docs.microsoft.com/en-us/microsoft-edge/progressive-web-apps-chromium/how-to/web-app-manifests) that is critical to making PWAs installable. [Functionally](https://docs.microsoft.com/en-us/microsoft-edge/progressive-web-apps-chromium/how-to/web-app-manifests), it _governs how your PWA looks and behaves when installed on device_ by defining the properties (key-value pairs) that characterize its appearance and behaviors. 


## A Sample PWA Manifest

Let's take a look at what a real manifest looks like using a [Sample PWA](https://aka.ms/learn-PWA/30Days-1.1/docs.microsoft.com/en-us/microsoft-edge/progressive-web-apps-chromium/demo-pwas). I'm using [DevTool Tips](https://devtoolstips.org/) - and I can view its manifest in two ways:
 ...
 * **From source.** Look for a [manifest.json](https://raw.githubusercontent.com/captainbrosset/devtools-tips/main/src/manifest.json) file. Apps may choose to use a `app_name.webmanifest` naming convention if they serve it with an `application/manifest+json` MIME type. 
...
![Placeholder for visual on manifest](_media/manifest-analogy.png)
...
But wait - there's more. The PWA can also list its resume in app stores just like native apps (publish to [e.g., Microsoft Store](https://docs.microsoft.com/en-us/microsoft-edge/progressive-web-apps-chromium/how-to/microsoft-store) for Windows devices). When users `Install` from app store, they just trigger the same interview process. **Installability and discoverability for the win!**
...
Create your manifest with any text editor or IDE - just make sure your JSON is valid. Start with a minimal manifest to get a working PWA. Then iteratively add new [members](https://developer.mozilla.org/docs/Web/Manifest) to improve PWA experience. Then add capability-driven attributes related to [Web APIs](https://developer.mozilla.org/en-US/docs/Web/API) you support.

Audit your PWA with tools like [PWABuilder](https://pwabuilder.com) to find, and fix, issues in your manifest so you can deliver a best-in-class PWA experience to users.

## What does a minimal Manifest need?

The [specfication](https://w3c.github.io/manifest/#examples) defines a number of _member_ attributes - but a suggested [minimal manifest](https://docs.microsoft.com/en-us/microsoft-edge/progressive-web-apps-chromium/how-to/web-app-manifests) should have at least these three - where *start_url* defines the entry point (default path shown) when app is launched on device.
...
A user-friendly description of the supported members can be read [here](https://developer.mozilla.org/docs/Web/Manifest) - let's see what our sample PWA is declaring for instance:
 * `short_name` - app name for constrained spaces (e.g., home screen)
 * `categories` - hints for stores or app catalogs. (See W3C [examples](https://github.com/w3c/manifest/wiki/Categories))
 * `display` - how much of browser UI does user see in app? 
 * `background_color` - placeholder tp show (before stylesheet loads)
...
For the first part, an actionable step to use audit tools like [PWABuilder](https://pwabuilder.com) that grade manifest quality and help fix issues interactively (see example below). Look for our [Week 3: Developer Tools](../dev-tools) coverage for more details.

![DevTools Tips - Manifest Audit](_media/dtt-manifest-report.png)

For the second part, we talked about two things

**Experimental members**. These are manifest members that are being evaluated for future inclusion, but subject to change. Browser platforms may offer early implementations under a [flag](https://docs.microsoft.com/en-us/microsoft-edge/progressive-web-apps-chromium/how-to/handle-urls#enable-url-handling) for evaluation - but keep in mind that these could change, and be unevenly supported across browsers.

 * _Example_: Our sample PWA declares a "url_handlers" member. This is an [experimental feature](https://docs.microsoft.com/en-us/microsoft-edge/progressive-web-apps-chromium/how-to/handle-urls) that allows the app to register itself as a launch target when user wishes to open associated URLs. There is now a proposal to replace this with a ["handle_links"](https://github.com/WICG/pwa-url-handler/blob/main/handle_links/explainer.md) member instead.

**Web capabilities**. The manifest can be used to declare support for using new [Web APIs](https://developer.mozilla.org/en-US/docs/Web/API) that unlock app access to rich platform hardware and features.

 * _Example:_ The ["share_target"](https://docs.microsoft.com/en-us/microsoft-edge/progressive-web-apps-chromium/how-to/web-app-manifests#identify-your-app-as-a-share-target) (used with the [Web Share API](https://developer.mozilla.org/en-US/docs/Web/API/Web_Share_API)), activates PWA ability to [share content with other apps](https://docs.microsoft.com/en-us/microsoft-edge/progressive-web-apps-chromium/how-to/share) like any other native app. Watch for Week 2, when we talk about advanced capabilities.
 ...
 * Inspect it's Manifest in DevTools. What [members](https://developer.mozilla.org/docs/Web/Manifest) did it define?
 * Audit it with [PWABuilder](https://pwabuilder.com). What gaps did it identify? 
 * Explore the Manifest. 
    - Are any experimental members in use? What do they do?
    - Are any capabilities declared? What [Web APIs](https://developer.mozilla.org/en-US/docs/Web/API) are behind them?
