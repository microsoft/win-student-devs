...
As you see, currently, there is no perfect way to manage the badge from the service worker. The discussion about this topic is ongoing, and there are [some proposals](https://github.com/w3c/badging/blob/main/explainer.md#possible-changes-to-the-push-api) about making Push API more relevant for app badges management.
...
Generate a set of so-called VAPID keys to authenticate a user device against the Messaging Service. You need one set of keys (public and private) per app. You can use either [web-push](https://www.npmjs.com/package/web-push) module or one of free online services. Using `web-push`:
...
After the user explicitly opt-in to receive push notifications (for example, by clicking a button in the UI), use `subscribe()` method of the `PushManager` interface (if it's available) of the active service worker registration:
...
You will find an extended code sample that also includes method for converting the `publicKey` to Uint8Array [here](https://docs.microsoft.com/en-us/microsoft-edge/progressive-web-apps-chromium/how-to/notifications-badges#step-2---subscribe-to-push-notifications).
...
The simpler way to send a notification from your NodeJS backend is using `sendNotification()` method from the [web-push](https://www.npmjs.com/package/web-push) module. There are similar libraries for [other platforms](https://github.com/web-push-libs/) available.
...
- [Voluntary Application Server Identification (VAPID) for Web Push](https://datatracker.ietf.org/doc/html/draft-ietf-webpush-vapid-00)
