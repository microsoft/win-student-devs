...
As you see, currently, there is no perfect way to manage the badge from the service worker. The discussion about this topic is ongoing, and there are [some proposals](https://github.com/w3c/badging/blob/main/explainer.md#possible-changes-to-the-push-api) about making Push API work better with badging.
...
Generate a set of "VAPID keys" to authenticate a user device against the Messaging Service. You need one set of keys (public and private) per app. You can use either [web-push](https://www.npmjs.com/package/web-push) module or one of the many free online services. Using `web-push`:
